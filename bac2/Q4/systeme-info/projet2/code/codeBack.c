...
void work(work_on_t *w , matrix_t *slot){
	...
	if(w->y!=0){
		...
		int c=0;
		for(int i = tx*(ty-1); i<tx*ty;i++){
			up[c]= slot->mat[w->x][w->y-1]->pi[i];
			c++;
		}
	}
	if(w->y!=NUMY-1){
		...
		for(int i=0 ;i<tx;i++){
			down[c]= slot->mat[w->x][w->y+1]->pi[i];
			c++;
		}
	}
	if(w->x!=0){
		...
		for(int i = 0 ; i<ty;i++){
			...
			left[c]= slot->mat[w->x-1][w->y]->pi[i*tx];
	...
	if(w->x!=NUMX-1){
		...
		int c=0;
		for(int i = 0 ; i<ty;i++){
			right[c]= slot->mat[w->x+1][w->y]->pi[(i*tx)+tx-1];
			c++;
		}
	}
			...
			if(cx==0 || cy==0 || cx==dx || cy==dy){
				...
				for(int i2=-1 ; i2<=1;i2++){
					for(int j2=-1 ; j2<=1;j2++){
						...
						vr = vr+(tab[i2+1][j2+1]/9 * (float)(w->data->pi[posi]->r));
					}
				}
				//printf("vb %f \n " , vb);
				ret[count]->b=(int)vb;
